...
import { PlusIcon, PencilSquareIcon, TrashIcon, ArrowUpTrayIcon, PhotoIcon, UserCircleIcon, CalendarIcon, ArrowLeftIcon } from '@heroicons/react/24/outline'
...
export default function AdminLivestock() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "livestock"), (snap) => {
      ...
      // ✅ Safe Sort Logic: Most recent at top, handles missing timestamps
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full p-3 bg-gray-50 rounded-xl text-xs border-none ring-1 ring-gray-100 text-center" 
                    value={form.image} 
                ...
                <button 
                  ...
                  className="w-full py-4 bg-emerald-900 text-white font-bold rounded-lg disabled:opacity-50"
                >
                  {loading ? 'Saving...' : 'Save Livestock'}
                </button>
                {editId && <button onClick={() => {setEditId(null); setForm({breed:'', name:'Pig', price:'', desc:'', specs:'', image:'', color:'bg-amber-600'})}} className="w-full mt-2 text-gray-400 text-xs font-bold">Cancel Editing</button>}
              </form>
            </div>

            {/* HERO SETTINGS */}
            <div className="bg-white p-4 md:p-8 rounded-lg shadow-sm border border-emerald-100">
              <h3 className="font-black text-emerald-900 mb-4 flex items-center gap-2"><PhotoIcon className="w-5 h-5"/> Hero Settings</h3>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    onChange={e => setHeader({...header, heroImage: e.target.value})} 
                  />
                </div>

                <button 
                  onClick={() => {setDoc(doc(db, "settings", "livestockPage"), header); toast.success("Header saved")}} 
                  disabled={uploadingHero}
          ...
          <div className="lg:col-span-2 space-y-4">
            <h2 className="text-xl md:text-2xl font-black text-emerald-900 mb-4 px-2">Live Catalog</h2>
