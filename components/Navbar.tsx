...
const Navbar = () => {
  ...
  // ✅ LOGIC FIX: Check for CEO UID OR Admin Collection
  useEffect(() => {
    const checkAdminStatus = async () => {
      const CEO_ID = process.env.NEXT_PUBLIC_ADMIN_ID;
      if (!user) { setIsAdmin(false); return; }

      // 1. CEO is always Admin (UID is the master key)
      if (user.uid === CEO_ID) { setIsAdmin(true); return; }

      // 2. Staff: Must have used 'password' login method
      ...
      if (usedPassword && user.email) {
        ...
        setIsAdmin(false); // Even if they are staff, if they use Google, they aren't 'Admin'
      }
    };
    checkAdminStatus();
  }, [user]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <Link href="/" className="flex items-center space-x-3 group">
            <div className="w-12 h-12 bg-gradient-to-br from-green-500 to-green-700 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
              <span className="text-white font-bold text-2xl">F</span>
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">FarmFresh</h1>
              <p className="text-xs text-green-600 -mt-1 font-medium">Premium Livestock</p>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {user ? (
              <div className="relative">
                <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center space-x-2 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors">
                  <img 
          ...
          <div className="flex md:hidden items-center">
            <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-lg text-gray-700 hover:bg-gray-100 transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100 space-y-4">
            ...
            {user ? (
              <div className="pt-4 border-t border-gray-100 space-y-3">
                ...
                {isAdmin && (
                  <Link href="/admin" onClick={() => setIsOpen(false)} className="w-full text-left px-4 py-3 text-amber-700 bg-amber-50/50 rounded-xl flex items-center gap-2 font-bold"><ShieldCheckIcon className="w-5 h-5" />Admin Dashboard</Link>
                )}
                <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-red-600 hover:bg-red-50 rounded-xl flex items-center gap-2"><ArrowRightStartOnRectangleIcon className="w-5 h-5" />Sign Out</button>
              </div>
            ) : (
